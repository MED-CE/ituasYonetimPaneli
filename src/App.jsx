...
const STORAGE_KEY = "istanbulls6064_v1";
const WORKSHOP_QR_TOKEN = "ISTANBULLS-6064-WORKSHOP-V1";
...
const initialData = {
  user: null,
  members: [
    { id: 1, name: "Ali Eren", role: "Yönetici", department: "Yönetim", email: "ali@istanbulls6064.com", phone: "", active: true },
...
const roles = ["Yönetici", "Kaptan", "Üye"];
const departments = ["Yönetim", "Mechanical", "Programming", "Electronics", "PR / Media", "CAD", "Drive Team", "Other"];
...
function Login({ onLogin }) {
  ...
  return (
    <div className="loginPage">
      ...
      >
        <img
          src="/istanbulls-logo.png"
          className="loginLogo"
          alt="Istanbulls 6064"
        />

        <div className="eyebrow">TEAM 6064</div>
          ...
          <>
            <label>
              ...
              <input
                ...
                placeholder="ali veya e-posta adresin"
            ...
            <small className="loginHint">
              Hesabın Supabase üzerinden doğrulanır.
            </small>
          </>
        ) : (
          <>
...
function App() {
  ...
  const canManage = role === "Yönetici" || role === "Kaptan";
  ...
  return (
    <div className="app">
      <aside className="sidebar">
        <div className="brand">
          <img src="/istanbulls-logo.png" className="logo" alt="Istanbulls 6064" />
          <div><strong>ISTANBULLS 6064</strong><span>Team Management</span></div>
        </div>
      ...
      <main className="main">
        <header className="topbar">
          <div className="mobileBrand"><img src="/istanbulls-logo.png" alt="" /></div>
  ...
  return (
    <>
      <section className="hero">
        <div className="heroLogo"><img src="/istanbulls-logo.png" alt="" /></div>
      ...
      <footer>© 2026 {data.settings.teamName}. <span>{data.settings.motto} 🐂 6064</span></footer>
    </>
  );
}
...
function CalendarPage({ data, update, canManage, currentUser, setData, flash }) {
  ...
  return <>
    ...
    <PageTitle title="Takvim" sub="Takım etkinliklerini planla, düzenle ve takip et." action={canManage && <button className="primary" onClick={()=>openNew()}>+ Etkinlik Ekle</button>} />
    ...
    <div className="calendar"><div className="calendarHead">{dayNames.map(d=><b key={d}>{d}</b>)}</div><div className="calendarBody">{days.map((day,i)=>{const key=day?`${month.getFullYear()}-${String(month.getMonth()+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`:"";const es=data.events.filter(e=>e.date===key);return <div className="calDay" key={i}><div className="dayTop"><span className="dayNum">{day}</span>{day&&canManage&&<button onClick={()=>openNew(day)}>+</button>}</div>{es.map(e=><div className="miniEvent" key={e.id} onClick={()=>setModal({type:"view",event:e})}><b>{e.title}</b><small>{e.start} - {e.end}</small><i>{e.location}</i></div>)}</div>})}</div></div>
    {modal?.type==="new" && <Modal title="Etkinlik Ekle" close={()=>setModal(null)}><EventForm form={form} setForm={setForm} save={save} close={()=>setModal(null)} /></Modal>}
...
function EventForm({form,setForm,save,close}) {
  return <><label>Etkinlik adı<input value={form.title} onChange={e=>setForm({...form,title:e.target.value})} placeholder="Örn. Robot Çalışması"/></label><div className="formGrid"><label>Tarih<input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})}/></label><label>Tür<select value={form.type} onChange={e=>setForm({...form,type:e.target.value})}><option>Toplantı</option><option>Robot Çalışması</option><option>Eğitim</option><option>Müsabaka</option><option>Etkinlik</option></select></label></div><div className="formGrid"><label>Başlangıç<input type="time" value={form.start} onChange={e=>setForm({...form,start:e.target.value})}/></label><label>Bitiş<input type="time" value={form.end} onChange={e=>setForm({...form,end:e.target.value})}/></label></div><label>Konum<input value={form.location} onChange={e=>setForm({...form,location:e.target.value})}/></label><label>Açıklama<textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label><div className="modalActions"><button className="ghost" onClick={close}>Vazgeç</button><button className="primary" onClick={save}>Kaydet</button></div></>;
}
...
function TasksPage({data,update,canManage,currentUser,flash}) {
  const [modal,setModal]=useState(false);
  const firstMember = data.members.find(m=>m.active);
  const [form,setForm]=useState({
    title:"",
    description:"",
    assignedId:firstMember?.id || "",
  ...
  const visibleTasks = canManage ? data.tasks : data.tasks.filter(t => String(t.assigned_id) === String(currentUser.id));
      ...
      <section className="panel" style={{
        ...
        <div style={{
          ...
          <div style={{flex:1, minWidth:240}}>
            <div className="eyebrow">ISTANBULLS 6064</div>
